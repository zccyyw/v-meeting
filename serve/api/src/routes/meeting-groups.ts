...
export async function meetingGroupRoutes(app: FastifyInstance, db: Db, redis: Redis) {
  ...
  app.post("/meetings/:id/invite", async (req, reply) => {
    ...
    const meetingId = Number((req.params as { id: string }).id);
    const body = z.object({
      userIds: z.array(z.number()).optional().default([]),
      deptIds: z.array(z.number()).optional().default([]),
    }).parse(req.body ?? {});
    ...
    for (const userId of body.userIds) {
      // 查用户信息
      const [userRows] = await db.query(
        `SELECT nick_name FROM sys_user WHERE user_id = ? AND del_flag = '0' LIMIT 1`,
        [userId],
      );
      const u = (userRows as { nick_name: string }[])[0];
      if (!u) continue;
      try {
        await db.query(
          `INSERT INTO meeting_invitations (meeting_id, user_id, display_name, status, invited_at)
           VALUES (?, ?, ?, 'pending', ${nowSql(db)})`,
          [meetingId, userId, u.nick_name],
        );
        invited++;
      } catch (e) {
        if (!isUniqueViolation(e)) throw e;
    ...
    for (const deptId of body.deptIds) {
      const [deptUsers] = await db.query(
        `SELECT user_id, nick_name FROM sys_user WHERE dept_id = ? AND del_flag = '0' AND status = '0'`,
        [deptId],
      );
      for (const u of deptUsers as { user_id: number; nick_name: string }[]) {
        try {
          await db.query(
            `INSERT INTO meeting_invitations (meeting_id, user_id, display_name, status, invited_at)
             VALUES (?, ?, ?, 'pending', ${nowSql(db)})`,
            [meetingId, u.user_id, u.nick_name],
          );
          invited++;
        } catch (e) {
          if (!isUniqueViolation(e)) throw e;
  ...
  app.get("/meetings/:id/invitations", async (req, reply) => {
    ...
    const meetingId = Number((req.params as { id: string }).id);
    const [rows] = await db.query(
      `SELECT i.*, u.nick_name AS user_nick_name, d.dept_name
       FROM meeting_invitations i
       LEFT JOIN sys_user u ON i.user_id = u.user_id
       LEFT JOIN sys_dept d ON u.dept_id = d.dept_id
       WHERE i.meeting_id = ?
       ORDER BY i.status, i.invited_at`,
      [meetingId],
    );
    ...
    const items = (rows as {
      ...
      deptId: r.dept_id ? Number(r.dept_id) : null,
      displayName: r.display_name ?? r.user_nick_name ?? "",
      deptName: r.dept_name ?? null,
      status: r.status,
  ...
  app.get("/meetings/:id/attendees", async (req, reply) => {
    ...
    const meetingId = Number((req.params as { id: string }).id);
    const [rows] = await db.query(
      `SELECT i.id, i.meeting_id, i.user_id, i.display_name, i.status, i.joined_at,
    ...
    const items = (rows as {
      ...
      meetingId: Number(r.meeting_id),
      userId: r.user_id ? Number(r.user_id) : null,
      displayName: r.display_name ?? r.nick_name ?? "",
